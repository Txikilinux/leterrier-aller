...
pico_status_t picodata_cbPutCh(register picodata_CharBuffer , picoos_char ch);
...
pico_status_t picodata_cbReset (register picodata_CharBuffer);
...
*/
pico_status_t picodata_cbGetItem(register picodata_CharBuffer ,
...
/* gets the speech data (without item head) from a CharBuffer in buf;
   ...
   set to the number of bytes gotten in buf; return values:
     ...
     PICO_EXC_BUF_OVERFLOW   <- buf not large enough
*/
pico_status_t picodata_cbGetSpeechData(register picodata_CharBuffer ,
        picoos_uint8 *buf, const picoos_uint16 blenmax,
        picoos_uint16 *blen);
...
*/
pico_status_t picodata_cbPutItem(register picodata_CharBuffer ,
        const picoos_uint8 *buf, const picoos_uint16 blenmax,
        picoos_uint16 *blen);

/* unsafe, just for measuring purposes */
picoos_uint8 picodata_cbGetFrontItemType(register picodata_CharBuffer );
...
void picodata_disposeProcessingUnit(
        picoos_MemoryManager mm,
        picodata_ProcessingUnit * );

picodata_CharBuffer picodata_getCbIn(picodata_ProcessingUnit );
picodata_CharBuffer picodata_getCbOut(picodata_ProcessingUnit );
pico_status_t picodata_setCbIn(picodata_ProcessingUnit , picodata_CharBuffer cbIn);
pico_status_t picodata_setCbOut(picodata_ProcessingUnit , picodata_CharBuffer cbOut);

/* protected */
typedef pico_status_t (* picodata_puInitializeMethod) (register picodata_ProcessingUnit , picoos_int32 mode);
typedef pico_status_t (* picodata_puTerminateMethod) (register picodata_ProcessingUnit );
typedef picodata_step_result_t (* picodata_puStepMethod) (register picodata_ProcessingUnit , picoos_int16 mode, picoos_uint16 * numBytesOutput);
typedef pico_status_t (* picodata_puSubDeallocateMethod) (register picodata_ProcessingUnit , picoos_MemoryManager mm);
