...
ExerciceOrdreAlpha1::ExerciceOrdreAlpha1(QWidget *parent, QString repertoireBase) :
...
//        m_config->setIniCodec("UTF-8");
        ...
        if(m_config->value("ordreAlphabetique/afficherMotBienPlaces").toBool()){
            JEVEUXCOLORIERETIQUETTESMALPLACEES=1;
        }
        else{
            JEVEUXCOLORIERETIQUETTESMALPLACEES=0;
        }
        if (m_config->value("ordreAlphabetique/exerciceActive").toBool() == false)
        {
            displayErrorMissingExercise(QString::fromUtf8("Ordre alphabétique"),parent);
            this->deleteLater();
        }
    }
    getAbeExerciceMessageV1()->setParent(m_parent);
    // Et on démarre la QStateMachine
    sequenceMachine->start();
}

ExerciceOrdreAlpha1::~ExerciceOrdreAlpha1()
{
    delete m_texte;
    delete m_config;
...
{
    if (m_localDebug) qDebug()<<"               AbulEduOrdreAlpha1::slotSequenceEntered()";

    setAbeNbExercices(m_config->value("ordreAlphabetique/exercice/size").toInt()); // Nombre d'exercices
    ...
    onPeutPresenterSequence = m_config->value("ordreAlphabetique/afficherConsigneSequence").toBool() || m_config->value("ordreAlphabetique/afficherTexteSequence").toBool();
...
{
    if (m_localDebug) qDebug()<<"               AbulEduOrdreAlpha1::slotPresenteSequenceEntered();";
...
//    getAbeExerciceTelecommandeV1()->ui->btnSuivant->setVisible(true);
    getAbeExerciceTelecommandeV1()->ui->btnSuivant->setEnabled(true);
    getAbeExerciceMessageV1()->abeWidgetMessageSetTitre(trUtf8("Ordre alphabétique"));
    if (m_config->value("ordreAlphabetique/afficherConsigneSequence").toBool())
    {
        QString debutTableau = "<tr valign=middle>";
        QString imagetete = "<td> " + QString(" <img src=\":/evaluation/neutre\"></td>");
        QString consigne = "<td align = right> " +  m_config->value("ordreAlphabetique/consigneSequence").toString() + "</td>";
        ...
        QString instructionFilePath = fi.absolutePath().replace("conf","data/instructions/oaConsigneSequence.ogg");
    ...
    if(m_config->value("ordreAlphabetique/afficherTexteSequence").toBool())
    {
...
void ExerciceOrdreAlpha1::slotExercicesEntered()
{
...
{
    if (m_localDebug) qDebug()<<" ++++++++ "<< __FILE__ <<  __LINE__ << __FUNCTION__<<m_numExercice;
    m_config->beginGroup("ordreAlphabetique");
        m_config->beginReadArray("exercice");
            m_config->setArrayIndex(m_numExercice);
            getAbeExerciceMessageV1()->abeWidgetMessageSetConsigne(m_config->value("consigneExercice").toString());
            qDebug()<<m_config->value("consigneExercice").toString();
        m_config->endArray();
    m_config->endGroup();
...
void ExerciceOrdreAlpha1::slotRealisationExerciceEntered()
{
    m_config->beginGroup("ordreAlphabetique");
    m_config->beginReadArray("exercice");
    m_config->setArrayIndex(m_numExercice);
    m_nbMotsARanger = m_config->value("nbMotsARanger").toInt();
    if (!m_config->value("nbMotsARanger").toString().isEmpty())
        setAbeNbTotalQuestions(m_config->value("nbMotsARanger").toString().toInt());
    m_numLevel = m_config->value("niveauDifficulte").toInt();
    if (m_localDebug) qDebug()<<" ExerciceOrdreAlpha1::slotRealisationExerciceEntered() "<<QString::number(m_numLevel-1);
    m_config->endArray();
    m_config->endGroup();
...
void ExerciceOrdreAlpha1::slotInitQuestionEntered()
{
    ...
    {
        qDebug()<<"------------------------------------------------------------------------------------";
        qDebug()<<" ++++++++ ExerciceOrdreAlpha1::slotInitQuestionEntered()" << m_numExercice;
        qDebug()<<sequenceMachine->configuration().toList();
        qDebug()<<"------------------------------------------------------------------------------------";
    }
    // on va récupérer les paramètes de l'exercice dans le fichier de conf module.conf

    m_listeMots = takeWords(m_config->fileName(),m_numExercice, m_nbMotsARanger,m_texte);
    setAbeExerciceName(trUtf8("Ordre Alphabétique - Exercice %1").arg(m_numExercice +1)); // Sera utilisé par le système de log
    setAbeNbTotalQuestions(1); // Une seule question par exercice dans ce cas
    ...
    abeStateMachineSetNbClicSurVerifier(0);

    // Ne pas effacer, sinon tant pis
    AbulEduCommonStatesV1::slotInitQuestionEntered();

    afficheMots(); // Comme son nom l'indique
}

QStringList ExerciceOrdreAlpha1::takeWords(const QString& unitPath, int exerciseNumber, int wordsNumber, AbulEduTextV1 *text){
    QStringList wordsList;
    QSettings* config = new QSettings(unitPath, QSettings::IniFormat);
    config->beginGroup("ordreAlphabetique"); // L'indentation avec {...} n'est là que pour la lisibilité
    {
        // On rentre dans l'exercice numéro m_numExercice
        config->beginReadArray("exercice"); // L'indentation avec {...} n'est là que pour la lisibilité
        {
...
/* -------------------------------------- Cas n°1 : mode Manuel -------------------------------------- */
            ...
            {
                qDebug()<<"Mode Manuel";
                wordsList = config->value("listeMots").toString().split(";",QString::SkipEmptyParts);
                if(wordsList.size()>1)
                {
                    int i = 0;
                    while(i < wordsNumber && wordsList.size()!=0)
                    {
                        int hasard = qrand()%wordsList.size();
                        QString mot=wordsList.takeAt(hasard);
                        wordsList.append(mot);
                        i++;
                    }
                    wordsList = AbulEduOutilsV1::abeoutilsSortListeMots(wordsList); // On trie la liste dans l'ordre alphabétique
                }
                qDebug()<<trUtf8("Mots à trier : ")<<wordsList;
            }
/* ------------------------------------ Cas n°3 : mode Automatique ------------------------------------ */
            else if (config->value("modeAutomatique").toBool())
            {
                qDebug()<<" ----> Troisieme cas de figure : aleatoire complet";
                QStringList listeMotsTexte = text->abetextMots().values();
                //if (m_localDebug) qDebug()<<listeTamponMotsTexte;
                int i = 0;
                while (wordsList.count() < wordsNumber && !listeMotsTexte.isEmpty())
                {
                    int hasard = qrand()%listeMotsTexte.size();
                    QString mot=listeMotsTexte.takeAt(hasard);
                    if (!wordsList.contains(mot))
                    {
                        wordsList.append(mot);
                    }
                    i++;
                }
                 wordsList = AbulEduOutilsV1::abeoutilsSortListeMots(wordsList);
                qDebug()<<trUtf8("Mots à trier : ")<<wordsList;
            }
            else if (config->value("modeIntelligent").toBool()){
                qDebug()<<"mode intelligent abandonné pour l'instant, trop de problèmes dûs aux mots à plusieurs natures ";
            }
        }
        config->endArray();
    }
...
void ExerciceOrdreAlpha1::slotQuestionEntered()
{
    ...
    {
        qDebug()<<"------------------------------------------------------------------------------------";
        qDebug()<<" ++++++++ ExerciceOrdreAlpha1::slotQuestionEntered()";
        qDebug()<<sequenceMachine->configuration().toList();
        qDebug()<<"------------------------------------------------------------------------------------";
    }
//    setDimensionsWidgets(); // Place les Widgets à leur place
    ...
    setHelpText(trUtf8("Les étiquettes sous le texte contiennent les mots à replacer. Déplace-les vers un trou afin de les positionner dans l'ordre alphabétique.")+"<br />"
                + trUtf8("Clique sur le bouton <b>Vérification</b> quand tu as fini ou sur le bouton <b>Solution</b> si tu n'y arrives pas..."));
}

void ExerciceOrdreAlpha1::slotAfficheCorrectionQuestionEntered()
{
    if (m_localDebug)
    {
        qDebug()<<"------------------------------------------------------------------------------------";
        qDebug()<<" ++++++++ ExerciceOrdreAlpha1::slotAfficheCorrectionQuestionEntered()";
        qDebug()<<sequenceMachine->configuration().toList();
        qDebug()<<"------------------------------------------------------------------------------------";
    }
...
void ExerciceOrdreAlpha1::slotFinCorrectionQuestionEntered()
{
    if (m_localDebug)
    {
        qDebug()<<"------------------------------------------------------------------------------------";
        qDebug()<<" ++++++++ ExerciceOrdreAlpha1::slotFinCorrigerQuestionEntered()";
        qDebug()<<sequenceMachine->configuration().toList();
        qDebug()<<"------------------------------------------------------------------------------------";
    }
    ...
    {
        setAbeLineLog(trUtf8("mots présentés en désordre"),
    ...
    if (m_localDebug)
    {
        if (m_answers.count() > 0)
        {
            ...
            qDebug()<<trUtf8("mots présentés en désordre");
...
void ExerciceOrdreAlpha1::slotAfficheVerificationQuestionEntered()
{
    if (m_localDebug)
    {
        qDebug()<<"------------------------------------------------------------------------------------";
        qDebug()<<" ++++++++ ExerciceOrdreAlpha1::slotAfficheVerificationQuestionEntered()";
        qDebug()<<sequenceMachine->configuration().toList();
        qDebug()<<"------------------------------------------------------------------------------------";
    }
    if (JEVEUXCOLORIERETIQUETTESMALPLACEES == 1)
    {
        setHelpText(trUtf8("La couleur <b>verte</b> indique les étiquettes <b>bien</b> placées les unes par rapport aux autres.")+"<br />"
...
void ExerciceOrdreAlpha1::slotFinVerificationQuestionEntered()
{
    if (m_localDebug)
    {
        qDebug()<<"------------------------------------------------------------------------------------";
        qDebug()<<" ++++++++ ExerciceOrdreAlpha1::slotFinVerificationQuestionEntered()";
        qDebug()<<sequenceMachine->configuration().toList();
        qDebug()<<"------------------------------------------------------------------------------------";
    }
...
void ExerciceOrdreAlpha1::slotFinQuestionEntered()
{
    if (m_localDebug)
    {
        qDebug()<<"------------------------------------------------------------------------------------";
        qDebug()<<" ++++++++ ExerciceOrdreAlpha1::slotFinQuestionEntered()";
        qDebug()<<sequenceMachine->configuration().toList();
        qDebug()<<"------------------------------------------------------------------------------------";
    }
...
void ExerciceOrdreAlpha1::slotBilanExerciceEntered()
{
    ...
    {
        qDebug()<<"------------------------------------------------------------------------------------";
        qDebug()<<" ++++++++ ExerciceOrdreAlpha1::slotBilanExerciceEntered()";
        qDebug()<<sequenceMachine->configuration().toList();
        qDebug()<<"------------------------------------------------------------------------------------";
    }
    clearScene();
...
void ExerciceOrdreAlpha1::slotBilanSequenceEntered()
{
    ...
    if (m_localDebug) qDebug()<<"               AbulEduOrdreAlpha1::slotBilanSequenceEntered();------";
    AbulEduCommonStatesV1::slotBilanSequenceEntered();
    // On dessine le bilan des exercices
    QString message = "";
...
void ExerciceOrdreAlpha1::slotBilanSequenceExited()
{
    getAbeExerciceMessageV1()->setVisible(false);
...
void ExerciceOrdreAlpha1::slotQuitter()
{
    if (m_localDebug) qDebug()<<"               AbulEduOrdreAlpha1::slotQuitter";
    AbulEduCommonStatesV1::slotQuitter();
}

void ExerciceOrdreAlpha1::slotAide()
{
    AbulEduStateMachineV1::slotAide();
}
//-------------------------------------------------------------------------
//                      La QstateMachine arrête ici
//-------------------------------------------------------------------------
...
void ExerciceOrdreAlpha1::afficheMots()
{
    clearScene();
...
void ExerciceOrdreAlpha1::placeMots()
{
    ...
    foreach(QGraphicsItem *m_item, listeItem){
        if(m_item->type()==AbulEduPieceV1::AbulEduPieceType){ // Faut être sûr que c'est un Mot (une abulEduPieceV1)
            ...
            do{ // On cherche une place libre
                ...
                m_item->setPos(x,y);
            }while(m_item->collidingItems().size() > 0); // On regarde s'il y a une étiquette en-dessous (évite le chevauchement)

        }
    }
}

void ExerciceOrdreAlpha1::placeCellules()
{
    ...
    if(listeItem.size()>0)
    {
        ...
        int nbLignes = int(m_nbMotsARanger / nbCellulesParLigne)+1;
...
void ExerciceOrdreAlpha1::resizeEvent(QResizeEvent *event)
{
    Q_UNUSED(event);
    if (m_localDebug) qDebug()<<"               AbulEduOrdreAlpha1::resizeEvent()";
    //        setDimensionsWidgets();
}
void ExerciceOrdreAlpha1::showEvent(QShowEvent *event)
{
    Q_UNUSED(event);
    if (m_localDebug) qDebug()<<"               AbulEduOrdreAlpha1::showEvent()";
    //        setDimensionsWidgets();
}
void ExerciceOrdreAlpha1::setDimensionsWidgets()
{
    if (m_localDebug) qDebug()<<"                ExerciceOrdreAlpha1::setDimensionsWidgets()---start";
    float ratio = abeApp->getAbeApplicationDecorRatio();
    ...
    if (m_localDebug) qDebug()<<"largeur"<<large<<"hauteur"<<haut;
    // Placement de l'aire de jeu
    gv_AireDeJeu->abeEtiquettesSetDimensionsWidget(QSize(large-250 * ratio, haut - 100 * ratio));
    gv_AireDeJeu->move((250 * ratio) / 2, 100 * ratio);
    ...
    // Redimmensionne le widget de consignes
    redimensionneConsigne();
    redimensionneBilan();

    // Recalcule l'emplacement des etiquettes et des cellules
    placeMots();
    placeCellules();

    AbulEduCommonStatesV1::setDimensionsWidgets();

    if (m_localDebug) qDebug()<<"                ExerciceOrdreAlpha1::setDimensionsWidgets()---end";
}
...
void ExerciceOrdreAlpha1::updateDropResultReceived(abe::ABE_EVAL eval)
{
    setHelpText(trUtf8("Oui, on fait comme ça.")+"<br />"
                +trUtf8("Continue en vérifiant bien l'ordre de tes mots."));
    ...
    if (m_receivedDropResults.contains(abe::evalD))
    {
        setHelpText(trUtf8("Oui, on fait comme ça, mais...")+"<br />"
                    +trUtf8("... je crois que tu vas avoir un petit souci."));
    }
}

void ExerciceOrdreAlpha1::slotLimitePieceX()
{
...
void ExerciceOrdreAlpha1::slotLimitePieceY()
{
    AbulEduPieceV1* p = (AbulEduPieceV1*) sender();
...
QMap<int, QStringList> ExerciceOrdreAlpha1::abeoutilsCreerListeMots(QStringList listeMots, int nbMots, abe::ABE_LEVEL niveau, QString nature, int profondeur)
{
...
int ExerciceOrdreAlpha1::verifieReponse()
{
    if (m_localDebug) qDebug()<<"          ExerciceOrdreAlpha1::verifieReponse()";
    ...
    QList<AbulEduPieceV1 *> listeEtiquettes;

    QMap<int, AbulEduPieceV1 *> ma_liste;
    ...
    foreach(AbulEduCelluleV1 *cellule, m_listeCellules)
    {
        if(cellule->abeCelluleHasPiece())
        {
            ma_liste.insert(cellule->objectName().toInt(), cellule->abeCelluleHasPiece() );
        }
    }
//Les cellules et pièces n'ont été insérées dans la QMap que si elles allaient par deux... je ne comprends pas le passage par le foreach ci-dessous

    foreach( AbulEduPieceV1 *etiq, ma_liste)
    {
        if(etiq != 0)
        {
            listeEtiquettes.append(etiq);

        }
    }
    if(listeEtiquettes.size()<m_listeMots.size()) //
    {
...
//        setDimensionsWidgets();
    ...
    {
        // Vérification du bon placement des mots dans la liste

        // --- Création d'un vecteur avec les rangs des étiquettes rencontrées
        QVector<int> rangEtiquettesProposees(listeEtiquettes.size());

        for (int j=0;j<listeEtiquettes.size();j++)
        {
            rangEtiquettesProposees[j] = ma_liste.value(j)->objectName().toInt();
            reponseDonnee.append(listeEtiquettes[j]->abePieceGetTexte());
        }
        ...
        nbreponsesFausses = listeEtiquettes.size() - listeRangsCroissants.size();

        // On met les étiquettes mal placées dans la bonne couleur
        if(JEVEUXCOLORIERETIQUETTESMALPLACEES==1)
        {
            for(int i=0;i< listeEtiquettes.size();i++)
            {
                // A ce moment, suite contient les indices des etiquettes bien placées
                if(listeRangsCroissants.contains(listeEtiquettes[i]->objectName().toInt()))
                {
                    // Le mot est dans la liste, on le met dans la couleur ok

                    listeEtiquettes[i]->abePieceSetMarge(0);
                    listeEtiquettes[i]->abePieceSetBrush(QColor(3,151,3,179));
                    listeEtiquettes[i]->abePieceSetRadius(0,0);
                    listeEtiquettes[i]->update();
                }
                else
                {
                    // Le mot n'est pas dans la liste, on le met dans la couleur faux
                    listeEtiquettes[i]->abePieceSetBrush(QColor(234,33,0,179));
                    listeEtiquettes[i]->abePieceSetRadius(0,0);
                    listeEtiquettes[i]->update();
                }
            }
            // Mise en couleur des cellules contenant une étiquette à sa place nominale
//            foreach(AbulEduCelluleV1 *cellule, m_listeCellules){
//                if(cellule->objectName() == cellule->abeCelluleHasPiece()->objectName()){
//                    cellule->setBrush(QColor(0,255,0));
//                }
//            }
        }
    }
    m_answers.append(QPair<QVariant,QVariant>(reponseDonnee.join(","),reponseAttendue.join(",")));
    m_percent = setAbeTeteForResult(nbreponsesFausses,m_nbMotsARanger);
...
//    setDimensionsWidgets();
    ...
    if (m_localDebug) qDebug()<<"reponses fausses : "<<nbreponsesFausses<<"mots a ranger : "<<m_nbMotsARanger;

    return nbreponsesFausses;
}

int ExerciceOrdreAlpha1::donneReponse()
{
    if (m_localDebug) qDebug()<<"          ExerciceOrdreAlpha1::donneReponse()";
    setAbeTeteTelecommande("rien");
    setDimensionsWidgets();
    ...
    foreach (AbulEduCelluleV1* cellule,m_listeCellules)
    {
        ...
        reponseAttendue << cellule->objectName();
    }
    m_answers.append(QPair<QVariant,QVariant>(" --- ",reponseAttendue.join(",")));
    return -1;
}

void ExerciceOrdreAlpha1::clearScene()
{
...
void ExerciceOrdreAlpha1::redimensionneConsigne()
{
    getAbeExerciceMessageV1()->abeWidgetMessageResize();
...
void ExerciceOrdreAlpha1::redimensionneBilan()
{
    getAbeExerciceMessageV1()->move((getAbeExerciceAireDeTravailV1()->width() - getAbeExerciceMessageV1()->width())/2,
                    ((getAbeExerciceAireDeTravailV1()->height() - getAbeExerciceMessageV1()->height())/2) - 200*abeApp->getAbeApplicationDecorRatio());
#ifdef Q_WS_MAC
