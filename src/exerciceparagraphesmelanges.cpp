...
ExerciceParagraphesMelanges::ExerciceParagraphesMelanges(QWidget *parent, QString repertoireBase) :
...
//        m_config->setIniCodec("UTF-8");
        ...
        if(m_config->value("paragraphesMelanges/afficherParagraphesBienPlaces").toBool()){
            JEVEUXCOLORIERETIQUETTESMALPLACEES=1;
        }
        else{
            JEVEUXCOLORIERETIQUETTESMALPLACEES=0;
        }
        if (m_config->value("paragraphesMelanges/exerciceActive").toBool() == false)
        {
            displayErrorMissingExercise(QString::fromUtf8("Paragraphes mélangés"),parent);
            this->deleteLater();
        }
    }
...
ExerciceParagraphesMelanges::~ExerciceParagraphesMelanges()
{
...
void ExerciceParagraphesMelanges::slotSequenceEntered()
{
    if (m_localDebug) qDebug()<<" ++++++++ "<< __FILE__ <<  __LINE__ << __FUNCTION__;

    setAbeNbExercices(m_config->value("paragraphesMelanges/exercice/size").toInt());
//    m_texte = new AbulEduTexte(m_config->value("fichierTexte").toString());
...
//    boiteTetes->setVisible(false);
    ...
    m_listeParagraphes = m_texte->abetextParagraphes().values(); // Liste des paragraphes
    ...
    onPeutPresenterSequence = m_config->value("paragraphesMelanges/afficherConsigneSequence").toBool() || m_config->value("paragraphesMelanges/afficherTexteSequence").toBool();
...
void ExerciceParagraphesMelanges::slotPresenteSequenceEntered()
{
    if (m_localDebug) qDebug()<<" ++++++++ "<< __FILE__ <<  __LINE__ << __FUNCTION__;
//    presentationSequence->assignProperty(getAbeExerciceTelecommandeV1()->ui->btnSuivant, "enabled", true);
    ...
    getAbeExerciceMessageV1()->abeWidgetMessageSetTitre(trUtf8("Paragraphes mélangés"));
    if (m_config->value("paragraphesMelanges/afficherConsigneSequence").toBool())
    {
        QString debutTableau = "<tr valign=middle>";
        QString imagetete = "<td> " + QString(" <img src=\":/evaluation/neutre\"></td>");
        QString consigne = "<td align = right> " +  m_config->value("paragraphesMelanges/consigneSequence").toString() + "</td>";
        ...
        QString instructionFilePath = fi.absolutePath().replace("conf","data/instructions/pamConsigneSequence.ogg");
    ...
    if(m_config->value("paragraphesMelanges/afficherTexteSequence").toBool()){
...
void ExerciceParagraphesMelanges::slotExercicesEntered()
{
...
void ExerciceParagraphesMelanges::slotPresentationExerciceEntered()
{
    if (m_localDebug) qDebug()<<" ++++++++ "<< __FILE__ <<  __LINE__ << __FUNCTION__;
    ...
    m_config->beginGroup("paragraphesMelanges");
        m_config->beginReadArray("exercice");
            m_config->setArrayIndex(m_numExercice);
            int paragrapheDebut = m_config->value("paragrapheDebut").toInt();
            int paragrapheFin = m_config->value("paragrapheFin").toInt();
            ...
            QString message = "";
            for(int i = paragrapheDebut; i<paragrapheFin+1; i++)
            {
                message = message + m_listeParagraphes[i] +"<br />";
            }
...
void ExerciceParagraphesMelanges::slotRealisationExerciceEntered()
{
    if (m_localDebug) qDebug()<<" ++++++++ "<< __FILE__ <<  __LINE__ << __FUNCTION__;
    ...
    setAbeExerciceName(trUtf8("Paragraphes mélangés - Exercice %1").arg(m_numExercice+1)); // Sera utilisé par le système de log
...
QMap<int,QString> ExerciceParagraphesMelanges::takeParagraphs(const QString& unitPath, int exerciseNumber, AbulEduTextV1* text){
    QMap<int,QString> paragraphs;
    QStringList allParagraphs = text->abetextParagraphes().values();
    QStringList paragraphNumbers;
    QSettings* config = new QSettings(unitPath, QSettings::IniFormat);
    config->beginGroup("paragraphesMelanges");
        config->beginReadArray("exercice");
            ...
            {
                qDebug()<<"Mode Manuel";
                paragraphNumbers = config->value("listeParagraphes").toString().split(";",QString::SkipEmptyParts);
                for(int i=0; i<paragraphNumbers.size();i++){
                    int num = paragraphNumbers.at(i).toInt();
                    paragraphs.insert(num-1,allParagraphs.at(num-1));
                }
                qDebug() << trUtf8("Paragraphes utilisés : ")<<paragraphNumbers;
            }
            else
            {
                qDebug()<<trUtf8("Mode Automatique");
                int nbParagraphesTires = 0;
                bool finAtteinte = false;
                QList<int> numParagraphePresents;
                QList<int> numParagrapheTires;
                int numParagrapheFin = config->value("paragrapheFin", allParagraphs.size()-1).toInt();
                int numParagrapheDebut = config->value("paragrapheDebut",0).toInt();
                for (int i=numParagrapheDebut;i<=numParagrapheFin;i++)
                {
                    numParagraphePresents << i;
                }
                qDebug()<<trUtf8("Numéros paragraphes possibles : ")<<numParagraphePresents;
                while (nbParagraphesTires < config->value("nombreParagraphes", qMin(4, allParagraphs.size())).toInt() && !finAtteinte)
                {
                    int numParagrapheTirage = rand() % (numParagraphePresents.count());
                    int numParagrapheUtilise = numParagraphePresents.takeAt(numParagrapheTirage);
                    paragraphs.insert(numParagrapheUtilise,allParagraphs.at(numParagrapheUtilise));
                    nbParagraphesTires++;
                    numParagrapheTires << numParagrapheUtilise;
                    if (numParagraphePresents.count()<=0)
                        finAtteinte = true;
                }
                qDebug()<<trUtf8("Numéros paragraphes tires : ")<<numParagrapheTires;
            }
        config->endArray();
    config->endGroup();
            return paragraphs;
...
void ExerciceParagraphesMelanges::slotInitQuestionEntered()
{
    if (m_localDebug) qDebug()<<" ++++++++ "<< __FILE__ <<  __LINE__ << __FUNCTION__<< "Exercice n°"<<m_numExercice;
    ...
    aireDeJeu->abelabelCreeAbulEduLabels(takeParagraphs(m_config->fileName(),m_numExercice, m_texte).values(),"background-color: #dfded3 ;border-radius: 10px;padding-left:10px;","background-color: #e0c1a9 ;border-radius: 10px;padding-left:10px;","background-color: #ff3b34 ;border-radius: 10px;padding-left:10px;");
...
void ExerciceParagraphesMelanges::slotQuestionEntered()
{
if (m_localDebug) qDebug()<<" ++++++++ "<< __FILE__ <<  __LINE__ << __FUNCTION__;
//    setDimensionsWidgets(); // Place les Widgets à leur place

    AbulEduCommonStatesV1::slotQuestionEntered();
    setHelpText(trUtf8("Déplace par un glisser-déposer avec la souris les étiquettes pour les remettre dans l'ordre normal du texte. Tu peux aussi en sélectionner une puis cliquer sur les <b>flèches</b> vertes pour la déplacer vers le haut ou le bas.")+"<br />"
                + trUtf8("Clique sur le bouton <b>Vérification</b> quand tu as fini ou sur le bouton <b>Solution</b> si tu n'y arrives pas..."));
}

void ExerciceParagraphesMelanges::slotAfficheCorrectionQuestionEntered()
{
    if (m_localDebug) qDebug()<<" ++++++++ "<< __FILE__ <<  __LINE__ << __FUNCTION__;
    ...
    {
        qDebug()<<"------------------------------------------------------------------------------------";
        qDebug()<<" ++++++++ ExerciceParagraphesMelanges::slotAfficheCorrectionQuestionEntered()";
...
void ExerciceParagraphesMelanges::slotFinCorrectionQuestionEntered()
{
    if (m_localDebug)
    {
        qDebug()<<"------------------------------------------------------------------------------------";
...
void ExerciceParagraphesMelanges::slotAfficheVerificationQuestionEntered()
{
    if (m_localDebug)
    {
        qDebug()<<"------------------------------------------------------------------------------------";
    ...
    {
        setHelpText(trUtf8("La couleur <b>verte</b> indique les paragraphes <b>bien</b> placés les uns par rapport aux autres.")+"<br />"
                    + trUtf8("Mais pas forcément à leur <b>place</b> définitive.")+"<br />"
                    + trUtf8("Tu peux bouger des paragraphes puis cliquer sur le bouton <b>Vérification</b> à nouveau."));
    }
    else
    {
        setHelpText(trUtf8("Certains paragraphes sont mal placés.")+"<br />"
...
void ExerciceParagraphesMelanges::slotFinVerificationQuestionEntered()
{
    if (m_localDebug)
    {
        qDebug()<<"------------------------------------------------------------------------------------";
...
void ExerciceParagraphesMelanges::slotFinQuestionEntered()
{
    if (m_localDebug)
    {
        qDebug()<<"------------------------------------------------------------------------------------";
...
void ExerciceParagraphesMelanges::slotBilanExerciceEntered()
{
    if (m_localDebug) qDebug()<<" ++++++++ "<< __FILE__ <<  __LINE__ << __FUNCTION__;
...
void ExerciceParagraphesMelanges::slotBilanSequenceEntered()
{
    if (m_localDebug) qDebug()<<" ++++++++ "<< __FILE__ <<  __LINE__ << __FUNCTION__;
...
void ExerciceParagraphesMelanges::slotBilanSequenceExited()
{
    getAbeExerciceMessageV1()->setVisible(false);
#ifdef Q_WS_MAC
    m_parent->repaint();
...
void ExerciceParagraphesMelanges::slotQuitter()
{
    if (m_localDebug) qDebug()<<"               ExerciceParagraphesMelanges::slotQuitter";
    AbulEduCommonStatesV1::slotQuitter();
}

void ExerciceParagraphesMelanges::slotAide()
{
...
void ExerciceParagraphesMelanges::resizeEvent(QResizeEvent *event)
{
    Q_UNUSED(event)
    if (m_localDebug) qDebug()<<"               ExerciceParagraphesMelanges::resizeEvent()";
//    setDimensionsWidgets();
}
void ExerciceParagraphesMelanges::showEvent(QShowEvent *event)
{
    Q_UNUSED(event)
    if (m_localDebug) qDebug()<<"               ExerciceParagraphesMelanges::showEvent()";
//    setDimensionsWidgets();
}
void ExerciceParagraphesMelanges::setDimensionsWidgets()
{
    if (m_localDebug) qDebug()<<" ++++++++ "<< __FILE__ <<  __LINE__ << __FUNCTION__<<" -- 1 --";
...
void ExerciceParagraphesMelanges::updateHelpTextBecauseMotion(AbuleduLabelV1 *etiquette)
{
    Q_UNUSED(etiquette);
...
int ExerciceParagraphesMelanges::verifieReponse()
{
    if (m_localDebug) qDebug()<<" ++++++++ "<< __FILE__ <<  __LINE__ << __FUNCTION__;
...
int ExerciceParagraphesMelanges::donneReponse()
{
    QStringList reponseAttendue;
    ...
    qDebug()<<aireDeJeu->abelabelListeAbuleduLabelsByNumOrdre();
    qDebug()<<aireDeJeu->abelabelListeAbuleduLabelsByNumPlace();
...
QMap<int, QStringList> ExerciceParagraphesMelanges::abeoutilsCreerListeMots(QStringList listeMots, int nbMots, abe::ABE_LEVEL niveau, QString nature, int profondeur)
{
...
void ExerciceParagraphesMelanges::redimensionneConsigne()
{
    getAbeExerciceMessageV1()->abeWidgetMessageResize();
...
void ExerciceParagraphesMelanges::redimensionneBilan()
{
    //question meld 20120221
    getAbeExerciceMessageV1()->abeWidgetMessageResize();
    getAbeExerciceMessageV1()->move((getAbeExerciceAireDeTravailV1()->width() - getAbeExerciceMessageV1()->width())/2,
